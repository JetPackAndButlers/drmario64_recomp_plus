...
constexpr auto res_default            = ultramodern::renderer::Resolution::Original2x;
...
namespace ultramodern {
    ...
    void from_json(const json& j, renderer::GraphicsConfig& config) {
        ...
        // Migrate deprecated/buggy auto resolution to a deterministic option.
        if (config.res_option == renderer::Resolution::Auto) {
            config.res_option = renderer::Resolution::Original2x;
        }
    }
}
...
bool save_general_config(const std::filesystem::path& path) {    
    ...
    zelda64::to_json(config_json["crt_scanlines_mode"], zelda64::get_crt_scanlines_mode());
...
void set_general_settings_from_json(const nlohmann::json& config_json) {
    ...
    zelda64::set_crt_scanlines_mode(from_or_default(config_json, "crt_scanlines_mode", zelda64::CrtScanlinesMode::On));
...
void zelda64::reset_cont_input_bindings() {
    ...
    assign_all_mappings(0, recomp::InputDevice::Controller, recomp::default_n64_controller_mappings);
}
...
bool load_controls_config(const std::filesystem::path& path) {
    ...
    if (version_it != config_json.end()) {
        ...
        for (size_t i = 0; i < 4; i++) {
            const bool player_exists =
                players_it != config_json.end() && players_it->is_array() && (players_it->size() > i);
